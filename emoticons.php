...
/**
* Finds smileys format and add images of smileys.
*
* @param string $str - the string that you want to filter
* @param bool $listing - if true it will return all the smilies
* @param int $perpage - if value it's not 0 the listing will be done on multiple pages
* @return string - the filtered string
*
*
*/

function smiley ($str, $listing = false, $perpage = 0) {
$str=str_replace("&quot;)","&quot; )",$str);
	$emoticons_url="./emoticons/";
    // $emoticon_size = '_big';
    $emoticon_size = '';
...
$replacements = array(
			  ...
			  '.t.' => '<img src="'. $emoticons_url.'FMfqC'.$emoticon_size.'.png" style="border:none;background:none;vertical-align:-25%;" alt="happy" />',
              
			  '.megusta.' => '<img src="'. $emoticons_url.'UT0wMCC'.$emoticon_size.'.png" style="border:none;background:none;vertical-align:-25%;" alt="happy" />',
              
			  '.ft.' => '<img src="'. $emoticons_url.'CrJQ4Wd'.$emoticon_size.'.png" style="border:none;background:none;vertical-align:-25%;" alt="happy" />',
              
			  '.omg.' => '<img src="'. $emoticons_url.'qtXbzo2'.$emoticon_size.'.png" style="border:none;background:none;vertical-align:-25%;" alt="happy" />',
        ...
        if($listing) {
            $html = "<table class='smiley'>"; 
            $extra = ''; // extra content in case of pagination
            $query = ''; // we keep all the data from $_GET
            $page = 0;
            if($perpage) { // a simple array pagination
                $page = (int)$_GET['page'] <= 0 ? 1 : (int)$_GET['page'];
                $total_results = count($replacements);
                $pages = ceil($total_results / $perpage);
                if($page > $pages)
                    $page = $pages;
                $start = ceil(($page - 1) * $perpage);
                
                $replacements = array_slice($replacements, $start, $perpage);
                ...
                for($i = 1; $i <= $pages; $i++)
                    if($i == $page)
                        ...
                        $extra .= " <a href='?".ltrim("$query&page=$i", "&")."'>$i</a> "; // we remove the first &
            }
